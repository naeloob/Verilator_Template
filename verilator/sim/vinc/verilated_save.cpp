...
void VerilatedSave::open(const char* filenamep) VL_MT_UNSAFE_ONE {
    ...
    if (VL_UNCOVERABLE(filenamep[0] == '|')) {
        ...
        m_fd = ::open(filenamep,
                      O_CREAT | O_WRONLY | O_TRUNC | O_LARGEFILE | O_NONBLOCK | O_CLOEXEC | O_BINARY, 0666); //Naeloob
...
void VerilatedRestore::open(const char* filenamep) VL_MT_UNSAFE_ONE {
    ...
    if (VL_UNCOVERABLE(filenamep[0] == '|')) {
        ...
        // cppcheck-suppress duplicateExpression
        m_fd = ::open(filenamep, O_CREAT | O_RDONLY | O_LARGEFILE | O_CLOEXEC | O_BINARY, 0666); //Naeloob
        if (VL_UNLIKELY(m_fd < 0)) {
            // User code can check isOpen()
            m_isOpen = false;
            return;
